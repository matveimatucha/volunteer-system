/**
 * Серверная часть системы регистрации волонтёров.
 *
 * Экспортирует две Cloud Functions (v2):
 *   - api          — HTTP API (Express), на него ведёт rewrite /api/** из Hosting
 *   - syncToSheets — Firestore-триггер: зеркалирует регистрации в Google Sheets
 *                    через Apps Script webhook (секрет SHEETS_WEBHOOK_URL)
 */
...
function registrationFromDoc(doc) {
    const data = doc.data();
    // cancelToken — секрет ссылки отмены, наружу не отдаём
    const { cancelToken, ...rest } = data;
    return { ...rest, registrationId: data.registrationId || doc.id };
}

/** Валидация и нормализация ответов анкеты с клиента. */
...
/** Проверка Firebase ID token и custom claim admin. */
...
/** Данные заявки для страницы отмены (по токену из ссылки). */
...
/* ================================================================
   Админские маршруты (Firebase Auth + custom claim admin)
   ================================================================ */
...
/** Перевод из листа ожидания в подтверждённые. */
...
adminRouter.get('/sheets/status', asyncHandler(async (req, res) => {
    const url = (SHEETS_WEBHOOK_URL.value() || '').trim();
    if (!url) {
...
adminRouter.post('/sheets/bulk-sync', asyncHandler(async (req, res) => {
    ...
    if (!response.ok) {
        logger.error('Sheets bulk_sync failed', { status: response.status });
...
// Через Hosting rewrite путь приходит с префиксом /api, при прямом вызове — без него
app.use('/api', router);
app.use('/', router);

app.use((req, res) => res.status(404).json({ error: 'NOT_FOUND' }));

exports.api = onRequest({ secrets: [SHEETS_WEBHOOK_URL] }, app);

/* ================================================================
   Триггер: синхронизация регистраций в Google Sheets
   ================================================================ */

exports.syncToSheets = onDocumentWritten(
    { document: 'registrations/{regId}', secrets: [SHEETS_WEBHOOK_URL], retry: true },
    async (event) => {
        const url = (SHEETS_WEBHOOK_URL.value() || '').trim();
        if (!url) return; // интеграция не настроена

        const before = event.data.before.exists ? event.data.before.data() : null;
        const after = event.data.after.exists ? event.data.after.data() : null;
        if (!after) return; // удаление — в таблице ничего не делаем
        if (after.source === 'legacy-import') return;
        ...
        if (after.status === REGISTRATION_STATUS.CANCELLED
            ...
            payload = {
                action: 'cancel',
                registrationId: after.registrationId || event.params.regId,
                cancelledAt: after.cancelledAt || new Date().toISOString()
            };
        } else if (!before || before.status !== after.status) {
            // новая заявка или смена статуса (например, перевод из листа ожидания)
            payload = buildSheetsRegisterPayload({
                ...after,
                registrationId: after.registrationId || event.params.regId
            });
        } else {
            return; // изменение без смены статуса — таблицу не трогаем
        }
        ...
        if (!response.ok) {
            // Бросаем ошибку, чтобы Cloud Functions повторила доставку
